...
def load_data(file_path):
    try:
        df = pd.read_excel("data/ASEAN immunisation against measles and DPT.xlsx")
        return df
...
with col1:
    if st.button("< Previous Page"):
           st.switch_page("pages/22_Education - Literacy Rate.py")
    # This is the subtitle (not clickable)
    st.caption("Education - Literacy Rate")
with col2:
    # This makes "Next Article >" clickable
    if st.button("Next Page >"):
           st.switch_page("pages/24_Education - Primary School Enrolment.py")
    # This is the subtitle (not clickable)
    st.caption("Education - Primary School Enrolment")
